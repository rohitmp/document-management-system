...
public class OKMMail implements MailModule {
	 ...
	 * @param dstId        Where the document will be stored. If null, mail it not stored.
	 */
	public Mail sendMailWithAttachments(String token, List<String> toRecipients, String subject, String body, List<String> docsId, String dstId)
			throws AccessDeniedException, PathNotFoundException, ItemExistsException, AutomationException, VirusDetectedException,
			UserQuotaExceededException, UnsupportedMimeTypeException, FileSizeExceededException, ExtensionException, RepositoryException,
			PrincipalAdapterException, DatabaseException, IOException, LockException {
		log.debug("sendMailWithAttachments({}, {}, {}, {}, {})", token, toRecipients, subject, body, docsId);
		Mail mail = sendMailWithAttachments(token, toRecipients, null, null, subject, body, docsId, dstId);
		log.debug("sendMailWithAttachments: {}", mail.toString());
		return mail;
	}
